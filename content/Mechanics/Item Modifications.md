Item Modifications can be performed by the blacksmith [[Olvir]] atop the [[Walking Marketplace]].
# Number of Upgrade Slots

| Rarity of Item       | Number of potential upgrades |
| -------------------- | ---------------------------- |
| Common               | 4                            |
| Uncommon             | 3                            |
| Rare                 | 2                            |
| Very Rare            | 1                            |
| Legendary / Artifact | 0                            |
# Upgrades
### Amphibious
- Gold Cost. 255GP
- Material Requirement. Kraken Gland
- Can it stack? No
- Armor. You can breathe underwater and swim at the same speed as your walking speed.
- Weapon. This weapon ignores all penalties for attacking underwater.
- Implement. Once per short rest, you may cast Water Breathing at third level without expending a spell slot.
- Wondrous Item. You can breathe underwater and swim at the same speed as your walking speed.
### Ancient
- Gold Cost. 400GP
- Material Requirement. An Ancient Dragon’s scale
- Can it stack? No
- Armor. Gain advantage on saving throws versus magical effects.
- Weapon. You may use your WIS modifier when making attacks and dealing damage with this weapon.
- Implement. Enemies have disadvantage at making WIS saving throws versus your spells.
- Wondrous Item. You gain advantage on WIS Saving throws
### Blighted
- Gold Cost. 270GP
- Material Requirement. A bone from an Undead knight
- Can it stack? Yes
- Armor. You gain +1 on CON saving throws.
- Stack: Increases by +1.
- Weapon. This weapon deals an additional 1d6 of Necrotic Damage.
	- Stack: Increases by 1d6.
- Implement. Once per long rest, you may activate this ability. When you deal damage with one of your spells, you gain 1d6 hitpoints.
	- Stack: Increases by 1d6.
- Wondrous Item. You gain +1 on CON saving throws
	- Stack: Increases by +1.
### Blinking
- Gold Cost. 310GP
- Material Requirement. A preserved snowflake
- Can it stack? No
- Armor. You may use your Bonus Action to immediately teleport up to 30ft to a position you can see. You may use this ability 3 times per long rest.
- Weapon. When you successfully hit an opponent with an attack using this weapon, you may immediately teleport up to 30ft to a position you can see. You may use this ability 3 times per long rest.
- Implement. Once per long rest, you may cast Blink at third level without expending a spell slot.
- Wondrous Item. You may use your Bonus Action to immediately teleport up to 30ft to a point you can see. You may use this ability 3 times per long rest.
### Charming
- Gold Cost. 300GP
- Material Requirement. A Rose from the [[Elphame|Feywild]]
- Can it stack? No
- Armor. When hit by a melee attack, as a reaction, you may add your CHA bonus to your AC for that attack.
- Weapon. You may use a bonus action to give your next attack a bonus, equal to your CHA modifier.
- Implement. When someone is charmed by you, you get an additional +5 for Charisma based-checks against them.
- Wondrous Item. You gain advantage on CHA saving throws
### Divine
- Gold Cost. 180GP
- Material Requirement. A Celestial’s feather
- Can it stack? Yes
- Armor. You may cast Cure Wounds at 1st level once per short rest without expending a spell slot.
	- Stack: Increases the level spell by 1 (without expending a spell slot).
- Weapon. This weapon deals an additional 1d6 of Radiant Damage.
	- Stack: Increases by 1d6.
- Implement. You may cast Bless at 1st level once per short rest without expending a spell slot.
	- Stack: Increases the level spell by 1 (without expending a spell slot).
- Wondrous Item. You may cast Bless at 1st level once per short rest without expending a spell slot.
	- Stack: Increases the level spell by 1 (without expending a spell slot).
### Enhanced
- Gold Cost. 150 GP
- Can it stack? Yes
- Armor. You gain +1 to your AC.
	- Stack: Increases by +1.
- Weapon. This weapon gains +1 to Hit and Damage rolls.
	- Stack: Increases by +1.
- Implement. This implement gains +1 to Spell DC and Spell Attack rolls.
	- Stack: Increases by +1.
- Wondrous Item. You gain +1 to your AC.
	- Stack: Increases by +1.
### Flaming
- Gold Cost. 165GP
- Material Requirement. Heart of a Fire Elemental
- Can it stack? Yes
- Armor. Once per long rest, you may activate this as a bonus action. Any enemy that ends their turn within 5 ft of you takes 1d6 fire damage.
	- Stack: Increases by 1d6.
- Weapon. This weapon deals an additional 1d6 of Fire Damage.
	- Stack: Increases by 1d6.
- Implement. Once per short rest, you may cast Hellish Rebuke at first level without expending a spell slot.
	- Stack: Increases the level spell by 1 (without expending a spell slot).
- Wondrous Item. Once per short rest, you may cast Hellish Rebuke at first level without expending a spell slot.
	- Stack: Increases the level spell by 1 (without expending a spell slot).
### Fluid
- Gold Cost. 410GP
- Material Requirement. A Water Elemental’s heart
- Can it stack? No
- Armor. You may use your Bonus Action to disengage
- Weapon. If you deal damage to a target using this weapon, they cannot take an attack of opportunity against you until the end of their next turn.
- Implement. Once per short rest, if you successfully cast a spell on an enemy within 5ft of you, you may teleport 30ft to a position you can see.
- Wondrous Item. You gain a climbing speed of 30ft and the ability to walk on water.
### Furious
- Gold Cost. 220GP
- Material Requirement. A vial of Hill Giant blood
- Can it stack? No
- Armor. When you are hit by an enemy attack, use your reaction to do 1d6 plus your proficiency mod Force Damage.
- Weapon. You may choose to roll with advantage on attack rolls using this weapon. If you choose to do so, all enemy attacks on you gain advantage until the beginning of your next turn.
- Implement. Once per round, you may reroll a Damage or Healing spell roll on a result of 1.
- Wondrous Item. Once per round, use your reaction to reduce non-magical damage by half.
### Immovable
- Gold Cost. 220GP
- Material Requirement. An Earth Elemental’s Heart
- Can it stack? No
- Armor. You gain advantage on CON saving throws.
- Weapon. When you hit an opponent with an attack using this weapon, you may move them in any direction by 5ft. You may use this 3 times per long rest.
- Implement. Once per long rest, you may cast Telekinesis without expending a spell slot.
- Wondrous Item. You cannot be knocked prone by enemy abilities or traps. You may still choose to go prone
### Light
- Gold Cost. 170 GP
- Material Requirement. Tentacle of an Astral Squid
- Can it stack? No
- Armor. You gain advantage on DEX Saving throws.
- Weapon. This weapon gains the Finesse property.
- Implement. Once per short rest, you may cast Featherfall without expending a spell slot.
- Wondrous Item. Once per short rest, you may cast Featherfall without expending a spell slot.
### Mastermind
- Gold Cost. 390GP
- Material Requirement. A Devil’s Brain
- Can it stack? No
- Armor. Removes any disadvantages for Stealth checks.
- Weapon. When using this weapon, you may use your INT modifier when making attacks and dealing damage.
- Implement. Enemies have disadvantage at making INT saving throws versus your spells.
- Wondrous Item. You gain advantage on INT Saving throws.
### Rapid
- Gold Cost. 200GP
- Material Requirement. A feather from a Pegasus
- Can it stack? Yes
- Armor. You gain an additional 10ft to your Walking speed.
	- Stack: Walking speed increases by an additional 5ft.
- Weapon. This weapon deals an additional 1d6 of Thunder Damage.
	- Stack: Increases by 1d6.
- Implement. Once per long rest, you may cast Haste at third level without expending a spell slot.
	- Stack: Increases the number of targets by 1.
- Wondrous Item. Once per long rest, you may cast Haste at third level without expending a spell slot.
	- Stack: Increases the number of targets by 1.
### Resilient
- Gold Cost. 320GP
- Material Requirement. The stones of a 1000-year ruin
- Can it stack? No
- Armor. Choose a damage type. This armor gains resistance to that type.
- Weapon. Once per short rest whilst wielding this weapon, you may spend your reaction to give yourself +3 to your AC until the beginning of your next turn.
- Implement. Choose a damage type. You gain advantage on saving throws against that damage type
- Wondrous Item. Choose a damage type. You gain advantage on saving throws against that damage type
### Shining
- Gold Cost. 350GP
- Material Requirement. A lock of hair from a Gold Elemental
- Can it stack? No
- Armor. Whenever you succeed on the Persuasion check, you may have the target charmed by you for the next hour.
- Weapon. When using this weapon, you may use your CHA modifier when making attacks and dealing damage.
- Implement. Enemies have disadvantage at making CHA saving throws versus your spells.
- Wondrous Item. You gain advantage on CHA Saving throws.
### Slaying
- Gold Cost. 320GP
- Material Requirement. A bone from the
- Chosen creature type
- Can it stack? Yes
- Armor. Choose a creature type other than humanoid. When a creature of that type hits you with a melee attack, the creature takes 2d6 force damage.
	- Stack: Pick an additional creature type.
- Weapon. Choose a creature type other than humanoid. When you successfully hit a creature of that type with this weapon, the creature takes an additional 3d6 weapon damage.
	- Stack: Pick an additional creature type.
- Implement. Choose a creature type other than humanoid. Whenever you cast a spell that affects the selected creature type that requires a saving throw, the creature has disadvantage on that save.
	- Stack: Pick an additional creature type.
- Wondrous Item. Choose a creature type other than humanoid. You gain advantage on any checks related to that creature, such as researching, tracking, and recalling information. In addition, once per short rest, you can automatically sense the location of any creatures of the selected type within 1 mile.
	- Stack: Pick an additional creature type.
### Sturdy
- Gold Cost. 270GP
- Material Requirement. A Storm Giant’s heart
- Can it stack? No
- Armor. You gain advantage on STR saving throws.
- Weapon. When you successfully hit an opponent with an attack using this weapon, they must make a DC 15 , STR check. On a fail, they are knocked prone. You may use this ability 3 times per long rest.
- Implement. You may cast Mage Armor once per short rest without expending a spell slot.
- Wondrous Item. You may cast Mage Armor once per short rest without expending a spell slot.
### Venomous
- Gold Cost. 270GP
- Material Requirement. A Basilisk’s tooth
- Can it stack? No
- Armor. You gain resistance to poison damage and are immune to the poison effect.
- Weapon. The first time you successfully hit an enemy using this weapon on your turn, they must succeed a DC 15 CON save. On a fail, they become poisoned.
- Implement. The first time you successfully hit an enemy whilst wielding this implement on your turn, they must succeed a DC 15 CON save. On a fail, they become poisoned.
- Wondrous Item. You gain resistance to poison damage and are immune to the poison effect.
### Vicious
- Gold Cost. 205 GP 
- Material Requirement. The tongue of a poet
- Can it stack? No
- Armor. You gain advantage on Intimidation checks.
- Weapon. When using this weapon, if you roll a 20 on your attack roll, the target takes an additional 7 damage of the weapon’s type.
- Implement. When wielding this implement, if you roll a 20 on your spell attack roll, the target takes an additional 7 damage of the spell’s damage type.
- Wondrous Item. You gain advantage on Intimidation checks.